                      ...
                      try {
                    	...
                    	if($target.is(".datagrid") && (cssAttrName==="padding-left" || cssAttrName==="padding-top")) {
	                      ...
	                      if(domObject.css2svg && domObject.css2svg[attribute]) {                      	  
	                    	...
	                    	if (cssAttrName == "fill" || cssAttrName == "stroke") {
	                    	  jQuery.each($target, function(index, value) {
	                    	    ...
	                    	    if (calculatedValue == "") {
	                    	      obj.attr("class", ((CSSclass != undefined) ? obj.attr("class") : "") + " svgRestore");
	                    	      if (style != undefined) obj.attr("style", style.replace(new RegExp(cssAttrName + ":#[a-fA-F0-9]* !important","")));
	                    	    }
	                    	    else {
	                    	      var styleValue = cssAttrName + ":" + calculatedValue;
	                    	      if (CSSclass != undefined) obj.attr("class", obj.attr("class").replace("svgRestore",""));
	                    	      
	                    	      if (style != undefined) {
		                    	    if (style.indexOf(cssAttrName + ":") > -1)
			                    	  obj.attr("style", style.replace(new RegExp(cssAttrName + ":#[a-fA-F0-9]*( !important)*",""), styleValue));
		                    	    else obj.attr("style", style + ";" + styleValue);
		                    	  }
	                    	      else obj.attr("style", styleValue);
	                    	    }
	                    	  });
	                    	}
	                    	else
                      ...
                      if (value.endsWith(".svg")) {
                        $.get(value, function(svg){
                    	  ...
                    	  if ($target.prop("tagName") == "IMG") {
                    		$target.parent().append("<div id='"+ id +"' class='" + $target.prop("class") + "' alt='" + $target.prop("alt") + "' style='" + $target.attr("style") + "'>");
                    		$target.remove();
                    		
                    		var item = $("#"+ id);
                    		item.html(svg);
                    		item.children().css({"width":width,"height":height});
                    		item.data(data);
                    		item.attr("systemname", value);
                    	  }
                    	  else {
                    		$target.html(svg);  
                        ...
                        if ($target.prop("tagName") == "DIV") {
                    	  var id = $target.prop("id");
                     	  $target.parent().append("<img id='"+ id +"' class='" + $target.prop("class") + "' alt='" + $target.prop("alt") + "' style='" + $target.attr("style") + "'>");
                		  $target.remove();
                		  $target = $("#"+id);
                        }
