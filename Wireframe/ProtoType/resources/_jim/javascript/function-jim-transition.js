      ...
      var deferred = new jQuery.Deferred(),
        ...
        startIn = function(){
            ...
            jimUtil.refreshPageMinSize();
  			...
  			//Set svgs fill:none to attributes
  			if (!page.hasClass("ui-scenario")) {
  			  var svgs = $("#simulation svg path, #simulation svg rect, #simulation svg circle, #simulation svg ellipse, #simulation svg line, #simulation svg polyline, #simulation svg polygon, #simulation svg use");
  			  var regexp = /fill:\s*none(?: !important)?/;
  			  jQuery.each(svgs, function(index, value) {
  				var obj = $(value);
  				var style = obj.attr("style");
  				if (style != undefined && obj.attr("fill") == undefined) {  
  				  var fill = style.match(regexp);
  				  if (fill != null && fill.length > 0) {
  				    obj.attr("style", style.replace(regexp, ""));
  				    obj.attr("fill", fill[0].substring(fill[0].lastIndexOf(":")+1, fill[0].length));
  				  }
  				} 
